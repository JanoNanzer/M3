...
function problemA () {
  ...
  .then(poema => blue(poema))
  let promise2 = promisifiedReadFile('poem-two/stanza-02.txt')
  .then(poema => blue(poema))

  Promise.all([promise1, promise2]).then(()=> console.log('done'))
}
...
function problemB () {
  ...
  // promise version
  // ???
  filenames = filenames.map(route => promisifiedReadFile(route).then(promesa=>blue(promesa)))
...
function problemC () {
  ...
  // promise version no secuencial
  filenames = filenames.map(route =>{
    return promisifiedReadFile(route)
  })
  Promise.all(filenames)
  .then(promesa=> promesa.forEach(promesa=>blue(promesa)))
  .finally(()=> console.log('done'))
  

  // promise version secuencial
